...
import com.chendroid.learning.utils.DisplayUtils;
...
public class CustomTabLayout extends HorizontalScrollView implements ViewPager.OnPageChangeListener {
    ...
    protected boolean mIsClickSwitchTab;
...
//        mAttributeHolder = new BaseAttributeHolder(this, R.styleable.SlidingTabLayout);
//        mAttributeHolder.save(attrs, defStyleAttr);
    ...
    private void obtainAttributes(Context context, AttributeSet attrs) {
        ...
        mMarginRight = ta.getDimension(R.styleable.SlidingTabLayout_tl_margin_right,0f);
    ...
    public void notifyDataSetChanged() {
        ...
        for (int i = 0; i < mTabCount; i++) {
            tabView = View.inflate(mContext, R.layout.custom_tab_layout, null);
    ...
    public void addNewTab(String title) {
        View tabView = View.inflate(mContext, R.layout.custom_tab_layout, null);
        if (mTitles != null) {
            mTitles.add(title);
        }
    ...
    protected void addTab(final int position, String title, View tabView) {
        ...
        if (tv_tab_title != null) {
            ...
            if (mTabWidth > 0) {
                ...
                tv_tab_title.getPaint().getTextBounds(tv_tab_title.getText().toString(), 0, tv_tab_title.getText().length(), txtBound);
                lp_tab = new LinearLayout.LayoutParams((int) Math.max(txtBound.width() + DisplayUtils.dpToPixel(getContext(), 18), mTabWidth), FrameLayout.LayoutParams.MATCH_PARENT);
            }
            //tab 间距加大
            if(mMarginRight!=0){
                lp_tab.setMargins(0,0, DisplayUtils.dpToPixel(getContext(),mMarginRight),0);
            }
...
//            v.setPadding((int) mTabPadding, v.getPaddingTop(), (int) mTabPadding, v.getPaddingBottom());
    ...
    public void onPageSelected(int position) {
        ...
        if (mIsClickSwitchTab) {
            mIsClickSwitchTab = false;
            return;
        }
            ...
            if (mIndicatorHeight > 0) {
                ...
                if (mIndicatorGravity == Gravity.BOTTOM) {
                    ...
                    mIndicatorDrawable.setBounds(paddingLeft + (int) mIndicatorMarginLeft + mIndicatorRect.left,
                            ...
                            paddingLeft + mIndicatorRect.right - (int) mIndicatorMarginRight+paddingRight,
...
//    }
        ...
        if (mHandler != null) {
            switch (ev.getAction()) {
                case MotionEvent.ACTION_MOVE:
                    if (tabHorizontalScrollListener != null)
    ...
    * */
    ...
    public static interface TabHorizontalScrollListener {

        int getScrollReordLimit();

        void onReceiveReord();
    }

    private TabHorizontalScrollListener tabHorizontalScrollListener;

    public void setTabHorizontalScrollListener(TabHorizontalScrollListener tabHorizontalScrollListener) {
        mHandler = new Handler();
        this.tabHorizontalScrollListener = tabHorizontalScrollListener;
    }
    ...
    private void process(int scrollX){
        if (tabHorizontalScrollListener != null) {
            int limit = tabHorizontalScrollListener.getScrollReordLimit();

            if (Math.abs(scrollX - lastX) > limit) {
                tabHorizontalScrollListener.onReceiveReord();
                lastX = scrollX;
            }
        }
    }
