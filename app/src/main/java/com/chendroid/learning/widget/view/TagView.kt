...
constructor(context: Context, attrs: AttributeSet? = null, defStyleAttr: Int = 0) :
    ...
    init {
        val typedArray =
                context.obtainStyledAttributes(attrs, R.styleable.TagViewStyle)

        tagText = typedArray.getString(R.styleable.TagViewStyle_tagText)
        textColor = typedArray.getColor(R.styleable.TagViewStyle_tagTextColor, context.resources.getColor(R.color.GBK05A))
        textSize = typedArray.getDimension(R.styleable.TagViewStyle_tagTextSize, 14.dp.toFloat())
        answerCount = typedArray.getString(R.styleable.TagViewStyle_tagCount)
        tagBackground = typedArray.getResourceId(R.styleable.TagViewStyle_tagBackground, R.drawable.tag_default_bg)

        tagStatus = typedArray.getInt(R.styleable.TagViewStyle_tagStatus, STATUE_DEFAULT)
    ...
    private fun updateTagTextReal() {
        if (TextUtils.isEmpty(answerCount) || answerCount.equals("0")) {
            ...
            tagTextView.text = context.getString(R.string.tag_text, tagText, answerCount)
        }
    }
    ...
    private fun updateBackgroundReal() {
        ...
        // 因为缺少对应的色组，只能手动调整透明度
        if (tagBackground == R.drawable.tag_selected_bg) {
    ...
    fun updateTagViewByStatus(status: Int) {
        when (status) {
            STATUE_DEFAULT -> {
                ...
                setTagBackGround(R.drawable.tag_default_bg)
            }
            STATUE_SELECTED -> {
                ...
                setTagBackGround(R.drawable.tag_selected_bg)
            }
            ...
            STATUE_JUST_SHOW -> {
                ...
                setTagBackGround(R.drawable.tag_more_bg)
            }
        }
    }
