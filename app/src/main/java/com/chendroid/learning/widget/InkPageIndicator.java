...
package com.chendroid.learning.widget;
...
public class InkPageIndicator extends View implements ViewPager.OnPageChangeListener,
    ...
    public void setDotDiameter(@FloatRange(from = 0.0F) float dp) {
        dotDiameter = DisplayUtils.dpToPixel(getContext(), dp);
        dotRadius = dotDiameter / 2;
        halfDotRadius = dotRadius / 2;
    }

    @SuppressWarnings({"unused", "SameParameterValue"})
    public void setDotGap(@FloatRange(from = 0.0F) float dp) {
        gap = DisplayUtils.dpToPixel(getContext(), dp);
    }

    @SuppressWarnings("unused")
    public void setAnimationDuration(@IntRange(from = 0) long duration) {
        animDuration = duration;
        animHalfDuration = animDuration / 2;
    }

    @SuppressWarnings("unused")
    public void setPageIndicatorColor(@ColorRes int colorRes) {
        unselectedColour = ContextCompat.getColor(getContext(), colorRes);
        unselectedPaint.setColor(unselectedColour);
        invalidate();
    }

    @SuppressWarnings("unused")
    public void setCurrentPageIndicatorColor(@ColorRes int colorRes) {
        selectedColour = ContextCompat.getColor(getContext(), colorRes);
        selectedPaint.setColor(selectedColour);
        invalidate();
    }
